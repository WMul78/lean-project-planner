...
export default function ImpactAnalysisListPage() {
  ...
  async function load() {
    ...
    try {
      ...
      console.error(e);
      alert(e?.message ?? "Failed to load impact analyses.");
  ...
  async function createNew() {
    ...
    try {
      ...
      const comp = await createLeanComponentInstance({ project: pr, componentType: "impact_analysis" });
      router.push(`/projects/${projectId}/lean/impact-analysis/${comp.id}`);
    } catch (e: any) {
      console.error(e);
      alert(e?.message ?? "Failed to create impact analysis.");
    }
  }
        ...
        <section className="mt-6 grid gap-3">
          {items.length === 0 ? (
            <div className="border rounded-xl p-4 text-sm text-gray-600">
              No impact analyses yet. Click <span className="font-medium">New analysis</span>.
            </div>
          ) : null}

          {items.map((it) => {
            const title = it.lean_impact_analysis?.title?.trim() || "Impact analysis";
            return (
              <div key={it.id} className="border rounded-xl p-4 flex items-center justify-between">
                ...
                <Button variant="outline" onClick={() => router.push(`/projects/${projectId}/lean/impact-analysis/${it.id}`)}>
                  Open
                </Button>
              </div>
            );
