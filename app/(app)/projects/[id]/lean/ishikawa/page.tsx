...
export default function IshikawaListPage() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          ...
          id,
          created_at,
          lean_ishikawa ( problem_statement )
        `
        )
      ...
      console.error(e);
      alert(e?.message ?? "Failed to load Ishikawa list.");
  ...
  async function createNew() {
    ...
    try {
      const pr = await loadProjectLean(projectId);
      const comp = await createLeanComponentInstance({ project: pr, componentType: "ishikawa" });
      router.push(`/projects/${projectId}/lean/ishikawa/${comp.id}`);
    } catch (e: any) {
      console.error(e);
      alert(e?.message ?? "Failed to create Ishikawa.");
    }
  }
        ...
        <section className="mt-6 grid gap-3">
          {items.length === 0 ? (
            <div className="border rounded-xl p-4 text-sm text-gray-600">
              No Ishikawa diagrams created yet. Click <span className="font-medium">New Ishikawa</span>.
            </div>
          ) : null}

          {items.map((it) => {
            const problem = it.lean_ishikawa?.problem_statement?.trim() || "Define the problem";
            return (
              <div key={it.id} className="border rounded-xl p-4 flex items-center justify-between">
                <div>
                  <div className="font-semibold line-clamp-1">{problem}</div>
                  <div className="text-xs text-gray-500">Created: {new Date(it.created_at).toLocaleString()}</div>
                </div>
                <Button variant="outline" onClick={() => router.push(`/projects/${projectId}/lean/ishikawa/${it.id}`)}>
                  Open
                </Button>
              </div>
            );
