...
export default function VsmListPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`id, created_at, lean_external_diagrams ( title, external_url )`)
      ...
      console.error(e);
      alert(e?.message ?? "Failed to load VSM list.");
  ...
  async function createNew() {
    if (!canEdit) {
      alert("External diagrams are available on the Pro plan.");
      router.push("/pricing");
      return;
    }
    ...
    try {
      const pr = await loadProjectLean(projectId);
      const comp = await createLeanComponentInstance({ project: pr, componentType: "vsm" });
      router.push(`/projects/${projectId}/lean/vsm/${comp.id}`);
    } catch (e: any) {
      console.error(e);
      alert(e?.message ?? "Failed to create VSM.");
    }
  }
        ...
        <section className="mt-6 grid gap-3">
          {items.length === 0 ? (
            <div className="border rounded-xl p-4 text-sm text-gray-600">
              No VSM links yet. Click <span className="font-medium">New</span>.
            </div>
          ) : null}

          {items.map((it) => {
            const title = it.lean_external_diagrams?.title?.trim() || "VSM (External)";
            const hasUrl = !!it.lean_external_diagrams?.external_url;
            return (
              <div key={it.id} className="border rounded-xl p-4 flex items-center justify-between">
                <div>
                  <div className="font-semibold">{title}</div>
                  <div className="text-xs text-gray-500">
                    {hasUrl ? "Linked" : "Not linked"} · Created: {new Date(it.created_at).toLocaleString()}
                  </div>
                </div>
                <Button variant="outline" onClick={() => router.push(`/projects/${projectId}/lean/vsm/${it.id}`)}>
                  Open
                </Button>
              </div>
            );
